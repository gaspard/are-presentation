...
const experiment = pointsExperiment({
  ...
  settings: {
    ...
    break2: s.break(),
    a: s.float("$a$", "croiss. proie", 1.0, (v) => v > 0),          
    b: s.float("$b$", "compét. proie", 0.01, (v) => v >= 0),         
    c: s.float("$c$", "cons. x→y", 0.05, (v) => v > 0),              
    d: s.float("$d$", "cons. x→z", 0.005, (v) => v > 0),             
    e: s.float("$e$", "mort. y", 0.2, (v) => v > 0),                 
    f: s.float("$f$", "rend. x→y", 0.6, (v) => v > 0),              
    g: s.float("$g$", "cons. y→z", 0.03, (v) => v > 0),              
    h: s.float("$h$", "mort. z", 0.15, (v) => v > 0),                
    i: s.float("$i$", "rend. x→z", 0.3, (v) => v > 0),               
    j: s.float("$j$", "rend. y→z", 0.7, (v) => v > 0),               
    break3: s.break(),
